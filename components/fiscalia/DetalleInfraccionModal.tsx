                      ...
                      <Section icon={BadgeCheck} title="Oficial que registró" iconBg="bg-emerald-50" iconColor="text-emerald-700">
                        <div className="flex items-start gap-3 p-3 rounded-lg bg-slate-50 border border-slate-200">
                          ...
                          {detalle.oficial.activo !== 'NO_DATA' && (
                            <span className={`shrink-0 text-[11px] font-medium px-2 py-0.5 rounded-full leading-none ${String(detalle.oficial.activo) === 'true'
...
function MapSection({ ubicacion }: { ubicacion: InfraccionUbicacion }) {
  ...
  const tabClass = (v: 'map' | 'street') =>
    `px-3 py-1 text-[11px] font-medium transition-colors duration-150 ${view === v
...
function StreetViewMap({ lat, lng, onStatus }: { lat: number; lng: number; onStatus: (s: 'ok' | 'unavailable') => void }) {
  ...
  useEffect(() => {
    ...
    const checkStatus = () => {
      ...
      if (status === 'OK') {
        onStatus('ok')
      } else if (status === 'UNKNOWN_ERROR' || status === 'ZERO_RESULTS') {
        onStatus('unavailable')
      }
    }
  ...
  return <div ref={ref} style={{ width: '100%', height: '100%', minHeight: 200 }} />
}
