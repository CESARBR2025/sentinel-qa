...
import { guardarOficioJuzgadoAction } from '@/lib/agente_juzgado/actions'

const inputClass = "w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 outline-none transition-all duration-150 placeholder:text-slate-400 hover:border-slate-300 focus:outline-none focus:border-emerald-700 focus:ring-2 focus:ring-emerald-700/10"
...
export default function CargarOficioSection({
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const result = await guardarOficioJuzgadoAction(fd)
    ...
    return (
        <div>
            <div className="px-5 py-3 flex items-center justify-between gap-3 border-b border-emerald-200/40 bg-emerald-50 rounded-t-xl">
                <div className="flex items-center gap-3">
                    <div className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0 bg-emerald-700">
                        <FileText size={14} strokeWidth={2.2} className="text-white" />
                    </div>
                    <div>
                        <h3 className="text-xs font-medium uppercase tracking-wider text-emerald-700">Registrar Oficio</h3>
                        <p className="text-[10px] text-emerald-600 mt-0.5">Folio: {folio || idInfraccion}</p>
                    </div>
                </div>
                {onClose && (
            ...
            <div className="p-5 space-y-5">
                <>
                    <div className="flex items-center justify-between">
                        <span className="inline-flex items-center gap-1.5 bg-emerald-50 text-emerald-700 text-[11px] font-medium px-2.5 py-1 rounded-full">
                        ...
                        <div className="flex items-center gap-1.5">
                            {Array.from({ length: totalPasos }).map((_, i) => {
                                const stepNum = i + 1
                                return (
                                    <span key={stepNum} className={stepNum === paso ? 'h-[7px] w-5 rounded-full bg-emerald-700' : stepNum < paso ? 'h-[7px] w-[7px] rounded-full bg-emerald-700 opacity-35' : 'h-[7px] w-[7px] rounded-full bg-slate-200'} />
                                )
                            })}
                        </div>
                    </div>
                    ...
                    {paso === 1 && (
                        <div className="space-y-4">
                            ...
                            <button onClick={() => { if (validarOficio()) setPaso(2) }} className="w-full inline-flex items-center justify-center gap-2 rounded-lg py-2.5 text-[13px] font-medium text-white bg-emerald-700 hover:bg-emerald-800 active:bg-emerald-900 active:scale-[0.99] shadow-sm transition-all duration-150">
                                <span>Continuar</span>
                        ...
                        <div className="space-y-4">
                            <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 space-y-3">
                                <div className="flex items-center gap-2">
                                    <div className="w-5 h-5 rounded flex items-center justify-center shrink-0 bg-emerald-100"><User size={11} strokeWidth={2.5} className="text-emerald-700" /></div>
                                    <p className="text-xs font-medium text-emerald-700">Datos del infractor</p>
                                </div>
                            ...
                            <button onClick={() => { if (validarInfractor()) setPaso(3) }} className="w-full inline-flex items-center justify-center gap-2 rounded-lg py-2.5 text-[13px] font-medium text-white bg-emerald-700 hover:bg-emerald-800 active:bg-emerald-900 active:scale-[0.99] shadow-sm transition-all duration-150">
                        ...
                        <div className="space-y-4">
                            <div className="space-y-2">
                                ...
                                <div className="inline-flex items-center rounded-md p-0.5 bg-white border border-slate-200">
                                    <button type="button" onClick={() => { setEsTitularState(true); setPaso(3 + pasoInfractorOffset) }} className={`px-4 py-1.5 rounded text-[13px] font-medium transition-all duration-150 ${esTitularState === true ? 'bg-emerald-700 text-white shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}>Sí</button>
                                    <button type="button" onClick={() => { setEsTitularState(false); setPaso(3 + pasoInfractorOffset) }} className={`px-4 py-1.5 rounded text-[13px] font-medium transition-all duration-150 ${esTitularState === false ? 'bg-emerald-700 text-white shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}>No</button>
                                </div>
                            </div>
                            <div className="flex items-center gap-3">
                        ...
                        <div className="space-y-4">
                            <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                                <div className="flex items-center gap-2 mb-3">
                                    <div className="w-5 h-5 rounded flex items-center justify-center shrink-0 bg-emerald-100"><User size={11} strokeWidth={2.5} className="text-emerald-700" /></div>
                                    <p className="text-xs font-medium text-emerald-700">Los datos del infractor se usarán como datos del titular</p>
                                </div>
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <button onClick={() => { if (validarArchivo()) setPaso(4 + pasoInfractorOffset) }} className="flex-1 inline-flex items-center justify-center gap-2 rounded-lg py-2.5 text-[13px] font-medium text-white bg-emerald-700 hover:bg-emerald-800 active:bg-emerald-900 active:scale-[0.99] shadow-sm transition-all duration-150">
                                    <span>Continuar</span><ArrowRight size={14} strokeWidth={2.5} />
                                </button>
                            </div>
                        </div>
                    ...
                    {paso === (3 + pasoInfractorOffset) && esTitularState === false && (
                        <div className="space-y-4">
                            <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 space-y-3">
                                <div className="flex items-center gap-2">
                                    <div className="w-5 h-5 rounded flex items-center justify-center shrink-0 bg-emerald-100"><User size={11} strokeWidth={2.5} className="text-emerald-700" /></div>
                                    <p className="text-xs font-medium text-emerald-700">Datos del titular (capturar)</p>
                                </div>
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <button onClick={() => { if (validarTitular()) setPaso(4 + pasoInfractorOffset) }} className="flex-1 inline-flex items-center justify-center gap-2 rounded-lg py-2.5 text-[13px] font-medium text-white bg-emerald-700 hover:bg-emerald-800 active:bg-emerald-900 active:scale-[0.99] shadow-sm transition-all duration-150">
                                    <span>Continuar</span><ArrowRight size={14} strokeWidth={2.5} />
                                </button>
                            </div>
                        </div>
                    ...
                    {paso === (4 + pasoInfractorOffset) && esTitularState === false && (
                        <div className="space-y-4">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <button onClick={() => { if (validarArchivo()) setPaso(5 + pasoInfractorOffset) }} className="flex-1 inline-flex items-center justify-center gap-2 rounded-lg py-2.5 text-[13px] font-medium text-white bg-emerald-700 hover:bg-emerald-800 active:bg-emerald-900 active:scale-[0.99] shadow-sm transition-all duration-150">
                                    <span>Continuar</span><ArrowRight size={14} strokeWidth={2.5} />
                                </button>
                            </div>
                        </div>
                    ...
                    {(paso === (4 + pasoInfractorOffset) && esTitularState === true) || (paso === (5 + pasoInfractorOffset) && necesitaCaptura) ? (
                        <div className="space-y-4">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <button onClick={handleSubmit} disabled={saving} className="flex-1 inline-flex items-center justify-center gap-2 rounded-lg py-2.5 text-[13px] font-medium text-white bg-emerald-700 hover:bg-emerald-800 active:bg-emerald-900 active:scale-[0.99] shadow-sm transition-all duration-150 disabled:bg-emerald-200 disabled:cursor-not-allowed">
    ...
    return (
        <div className="space-y-1">
            ...
            <button
                ...
                className={`w-full rounded-lg border-2 border-dashed p-4 flex flex-col items-center gap-2 transition-colors cursor-pointer ${archivo ? 'bg-emerald-50 border-emerald-300' : error ? 'bg-red-50 border-red-300' : 'bg-slate-50 border-slate-200 hover:border-slate-300'}`}
            >
                {archivo ? (
                    <>
                        <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-emerald-700"><FileText size={16} strokeWidth={2} className="text-white" /></div>
