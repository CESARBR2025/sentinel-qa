...
import type { ViaInfraccionDetalle } from '@/lib/agente_juzgado/types'
...
const STATUS_TABS: { key: EstatusJuzgado; label: string; icon: typeof Clock; color: string; accent: string; bg: string }[] = [
    ...
    { key: 'LIBERADO_POR_JUZGADO', label: 'Liberados', icon: CheckCircle2, color: '#22C55E', accent: '#166534', bg: '#F0FDF4' },
]
...
const STATUS_BADGE: Record<string, { bg: string; text: string; dot: string; label: string }> = {
    ...
    EN_PROCESO_JUZGADO: { bg: '#dbdfe5', text: '#172844', dot: '#3e5171', label: 'En Proceso' },
    LIBERADO_POR_JUZGADO: { bg: '#DCFCE7', text: '#166534', dot: '#22C55E', label: 'Liberada' },
...
export default function JuzgadoDashboard({
    ...
    const estadisticas = useMemo(() => {
        ...
        const liberadas = data.filter(
            x =>
                x.estatus === 'REGISTRADA' &&
                ['MESA_DE_CONTROL_PENDIENTE_DOCS', 'LIBERADA_POR_ACCIDENTE', 'LIBERADA_POR_DELITO'].includes(x.estatus_dependencia)
        ).length
    ...
    function exportarCSV() {
        ...
        a.href = url
        a.download = `juzgado-${filtro}-${new Date().toISOString().split('T')[0]}.csv`
        a.click()
    ...
    async function iniciarRevision() {
        ...
        try {
            const response = await fetch('/api/agente_juzgado/iniciarProceso', {
                method: 'PATCH',
    ...
    async function handleFinalizarProceso(id: string) {
        setFinalizandoId(id)
        try {
            const res = await fetch('/api/agente_juzgado/finalizarProceso', {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                    ...
                    <div className="rounded-xl border overflow-hidden bg-white border-slate-200 shadow-card">
                        <div className="px-5 py-3 border-b flex items-center justify-between border-slate-100 bg-slate-50">
                            ...
                            <div className="flex items-center gap-3">
                                <button
                                    onClick={exportarCSV}
                                    className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-[11px] font-medium text-emerald-700 bg-emerald-50 border border-emerald-200 hover:bg-emerald-100 transition-colors"
                                >
                        ...
                        <div className="px-5 py-2.5 border-b flex items-center gap-3 border-slate-100 bg-white flex-wrap">
                            <div className="flex items-center gap-2 flex-1 min-w-0">
                                ...
                                {busqueda && (
                                    <button
                                        ...
                                        className="text-[10px] font-medium uppercase tracking-wider text-emerald-700 hover:text-emerald-800 transition-colors shrink-0"
                                    >
                                        Limpiar
                                    </button>
                                )}
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                                ...
                                <input
                                    ...
                                    className="w-[130px] rounded-md border border-slate-200 bg-white px-2 py-1.5 text-xs text-slate-900 outline-none focus:border-emerald-700 focus:ring-2 focus:ring-emerald-700/10"
                                />
                                        ...
                                        return pages.map((p, i) =>
                                            p === '...' ? (
                                                ...
                                                <button
                                                    key={p}
                                                    onClick={() => setPagina(p)}
                                                    className={`w-7 h-7 text-xs font-medium rounded-md transition-colors ${p === paginaSegura ? 'bg-emerald-700 text-white' : 'text-slate-600 hover:bg-slate-100'}`}
                                                >
                                                    {p}
                                                </button>
                                            )
