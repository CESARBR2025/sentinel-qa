...
import { User, CheckCircle2, Loader2, AlertCircle, ArrowRight, X } from 'lucide-react'
...
type FieldName = 'infrNombre' | 'infrAppaterno' | 'infrApmaterno' | 'infrCurp' | 'infrCorreo'
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-500 mt-0.5" style={{ fontFamily: "'JetBrains Mono',monospace" }}>
                  CAPTURA LOS DATOS DEL INFRACTOR PARA ENVIAR LINK DE PAGO
                </p>
              </div>
            </div>
            <button
...
function DatosForm({ infraccionId, onSuccess, onClose }: Props) {
  ...
  const handleBlur = useCallback((name: FieldName, value: string) => {
    markTouched(name)
    const trimmed = value.trim()
    if (!trimmed && name !== 'infrApmaterno' && name !== 'infrCorreo') {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await capturarInfractorInfraccionesAction({
        ...
        nombre_titular: infrNombre.trim().toUpperCase(),
        appaterno_titular: infrAppaterno.trim().toUpperCase(),
        apmaterno_titular: infrApmaterno.trim().toUpperCase(),
        curp_titular: infrCurp.trim().toUpperCase(),
        correo_titular: infrCorreo.trim(),
      })
      if (!result.success) {
