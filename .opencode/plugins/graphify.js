// graphify + context-loader OpenCode plugin
    ...
    "tool.execute.before": async (input, output) => {
      ...
      if (isNewTask && existsSync(LOADER_SCRIPT)) {
        try {
          ...
          if (ctx.trim()) {
            // Prepend context output to the first bash command
            output.args.command =
      ...
      // Fallback: remind about graphify on bash commands
      if (!graphReminded) {
        const cmdHint =
          existsSync(SKILL_PATH)
            ? `echo "[graphify] Consultar grafo con: graphify query \\"<pregunta>\\". Para contexto automático: skill context-loader"`
            : `echo "[graphify] Consultar grafo con: graphify query \\"<pregunta>\\""`;
        output.args.command =
