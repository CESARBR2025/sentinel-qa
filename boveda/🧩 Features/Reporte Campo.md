...
**Cómo se entera el oficial de una asignación**: al ejecutar `createDespacho`/`enviarRefuerzos` (`lib/incidentes/actions.ts`), justo después del `COMMIT` se emite `despacho.asignado`/`despacho.refuerzos` (`lib/notificaciones/catalogo.ts`) dirigido por `usuarios` a los `user_id` de `ofi_oficiales` de los elementos asignados con cuenta activa. La entrega es 100% in-app vía `CampanillaNotificaciones.tsx` (polling cada 45s, solo con la pestaña visible, sonido + badge) — no hay push nativo del navegador ni del sistema operativo (sin service worker, sin `Notification()`), decisión de arquitectura documentada en [[Plan - Sistema de Notificaciones]] por costo en el modelo serverless. Clic en la notificación lleva a `/oficial/despachos/{incidenteId}`. Elementos sin `oficial_id`/`user_id` resuelto (personal externo sin cuenta) no reciben nada — gap silencioso, sin aviso a nadie. Ver [[911]] regla 18, [[Notificaciones]].

**Navegación en vivo estilo DiDi (reemplaza los botones manuales)**: al abrir `/oficial/despachos/[id]` con un incidente que tiene `latitud`/`longitud`, el oficial ve la pantalla "INICIAR NAVEGACIÓN" (`components/oficial/navegacion/NavegacionDespacho.tsx`). Al confirmar, se dispara `marcarEnCaminoOficial` (idempotente, guarda `hora_salida` por `COALESCE`) y se pide el permiso de GPS. El mapa calcula la ruta real por calles con **Google Directions API** desde la posición GPS del oficial (watchPosition propio de alta precisión, independiente del heartbeat de `OficialUbicacionTracker.tsx`) y la redibuja solo si el oficial se desvía >150m del trazo y pasó ≥60s del último recálculo (control de costo de Directions). La llegada se detecta por **geofence de 80m** (Haversine contra el destino) y dispara `marcarEnSitioOficial` (cambia `incidentes.estatus` a `en_sitio`, puebla `hora_salida`/`hora_llegada`); hay un botón manual "✓ LLEGUÉ" siempre visible como respaldo (protegido contra doble disparo por ref). `emitir('despacho.en_camino'/'despacho.en_sitio')` avisa al despachador (ver [[Notificaciones]]). **Fallback sin coordenadas**: los botones manuales originales (`MarcarEnCaminoButton.tsx` + `MarcarEnSitioButton.tsx`) se conservan y se usan solo para incidentes sin `latitud`/`longitud` (badge "SIN COORDENADAS — REGISTRO MANUAL"). Decisión de negocio: el oficial reporta sus propios `hora_salida`/`hora_llegada` — el despachador no tiene botones para esto en `TablonDespacho.tsx` (solo lectura). Ahora, con GPS real, esos momentos se capturan solos en el instante correcto en vez de depender de que el oficial toque un botón.
...
| `components/oficial/navegacion/NavegacionDespacho.tsx` | Navegación en vivo (mapa + ruta Directions API + watchPosition + geofence de llegada) para incidentes con coordenadas |
| `components/oficial/MarcarEnCaminoButton.tsx` / `MarcarEnSitioButton.tsx` | Fallback manual para incidentes sin `latitud`/`longitud` |
