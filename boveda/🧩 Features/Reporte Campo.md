...
| `app/oficial/despachos/page.tsx`, `[id]/page.tsx` | Vista "Mis Despachos" — asignaciones activas; `[id]` arma Header + Body y dentro del body solo la navegación a altura completa (transición embebida a `FormularioRecorrido` al llegar) |
| `components/oficial/DespachoContent.tsx` | Orquestador cliente de `/oficial/despachos/[id]`: rama `en_sitio` → `FormularioRecorrido` embebido; caso normal → `NavegacionDespacho` a `flex: 1` |
| `components/oficial/navegacion/NavegacionDespacho.tsx` | Navegación en vivo (mapa + ruta Directions API + watchPosition + geofence de llegada) |
