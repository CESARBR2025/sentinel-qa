...
10. Cada unidad despachada (`incidente_despacho_unidades`) registra su propia `hora_salida`/`hora_llegada`, capturadas desde el tablón de despacho (tab "En Despacho") vía `marcarHoraUnidadDespacho`, o inferidas automáticamente (`COALESCE`, sin pisar lo manual) al marcar "en sitio" o al cerrar el reporte de campo — ver [[Reporte Campo]]
...
15. `FormRondinEscalado.tsx` (`/oficial/rondin`) usa ahora el mismo catálogo jerárquico completo que ciudadano/WhatsApp (`getCatalogos()` de `lib/911/service`, no el degradado `lib/oficial/service::obtenerCatalogos()` que solo traía `id`+`nombre`) — cascada Tipo→Subtipo→Incidente con código de 5 dígitos visible y prioridad autocompletada con ajuste manual opcional. Esto también corrigió un bug silencioso: el aviso de "Tipo Improcedentes" nunca se mostraba en este formulario porque el catálogo degradado no traía la columna `codigo` (el backend igual bloqueaba el despacho correctamente, solo la advertencia visual fallaba). **Pendiente de decidir**: `FormularioRecorrido.tsx` (cierre del reporte de campo) usa el mismo catálogo degradado, pero ahí `ofi_tipo_incidente`/`ofi_tipo_emergencia`/`ofi_prioridad` son columnas de texto libre (snapshot narrativo, no FK) — alinearlo al mismo nivel implicaría un cambio de modelo de datos más grande, no solo cambiar la fuente del catálogo.
