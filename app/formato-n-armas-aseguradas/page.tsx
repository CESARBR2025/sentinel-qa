...
export default async function FormatoNArmasAseguradasPage() {
  ...
  return (
    <div style={pageWrap}>
      ...
      <SubHeader backHref="/envio-de-formatos" backLabel="Envío de Formatos" title="Armas Aseguradas" user={user} />
      ...
      <main style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 48px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 32 }}>
          <div>
            ...
            <h1 style={{ fontFamily: 'Barlow Condensed', fontSize: 36, fontWeight: 800, color: '#0f172a', margin: '4px 0 0 0', textTransform: 'uppercase' }}>Armas de Fuego Aseguradas</h1>
            <div style={{ width: 64, height: 3, background: '#2563eb', marginTop: 12 }} />
          </div>
          <Link href="/formato-n-armas-aseguradas/nuevo" style={{ fontFamily: 'JetBrains Mono', fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.15em', background: '#0f172a', color: '#ffffff', padding: '12px 24px', textDecoration: 'none', borderRadius: 2, display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <tbody>
              ...
              {registros.map(r => (
                <tr key={r.id} style={{ borderBottom: '1px solid #e2e8f0' }}>
                  ...
                  <Td>{r.matricula ?? '—'}</Td>
                  <Td>{r.calibre ?? '—'}</Td>
                  <Td><Link href={`/formato-n-armas-aseguradas/${r.id}`} style={{ fontFamily: 'JetBrains Mono', fontSize: 10, color: '#2563eb', textDecoration: 'none' }}>EDITAR</Link></Td>
                </tr>
              ))}
            </tbody>
          </table>
