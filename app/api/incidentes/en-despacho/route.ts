...
export async function GET(_req: NextRequest) {
  ...
  const lista = await query(
    `SELECT i.id, i.folio, i.canal, i.estatus, i.fecha_hora_inicio AS "fechaHoraInicio", i.calle, i.colonia, i.descripcion, cti.nombre AS "tipoIncidente", cp.nombre AS prioridad, u.name AS "capturadoPor", d.id AS "despachoId", d.fecha_hora_despacho AS "fechaHoraDespacho" FROM incidentes i LEFT JOIN cat_tipos_incidente cti ON i.tipo_incidente_id = cti.id LEFT JOIN cat_prioridades cp ON i.prioridad_id = cp.id LEFT JOIN users u ON i.capturado_por = u.id LEFT JOIN incidente_despacho d ON i.id = d.incidente_id WHERE i.estatus = 'en_despacho' ORDER BY i.creado_en DESC LIMIT 100`,
  )
