...
import { obtenerFge, actualizarFge } from '@/lib/reportes/formato-n-fge-service'
...
export async function GET(
  ...
  const registro = await obtenerFge(id)
...
export async function PATCH(
  ...
  try {
    await actualizarFge(id, data)
    return NextResponse.json({ success: true })
  } catch (err: unknown) {
    const pgErr = err as { code?: string; message?: string }
    const isDuplicate = pgErr?.code === '23505' || (pgErr?.message || '').includes('duplicate key')
    if (isDuplicate) {
      return NextResponse.json({ error: 'Ya existe otro reporte FGE con esa fecha y periodo.' }, { status: 409 })
    }
