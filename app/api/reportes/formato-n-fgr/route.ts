...
import { listarFgr, crearFgr, obtenerFgrPorFechaPeriodo, Periodo } from '@/lib/reportes/formato-n-fgr-service'
...
export async function GET(req: NextRequest) {
  ...
  const registros = await listarFgr(periodo || undefined)
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (isDuplicate) {
      const existente = await obtenerFgrPorFechaPeriodo(body.fecha, body.periodo)
      return NextResponse.json({
        error: 'Ya existe un reporte FGR para esta fecha y periodo.',
