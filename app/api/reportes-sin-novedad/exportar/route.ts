...
export async function GET(req: NextRequest) {
  ...
  const COLS = [
    { header: 'Reporte',            key: 'reporte',            width: 20 },
    { header: 'Nombre Reportante',  key: 'nombreReportante',   width: 30 },
    { header: 'Teléfono',           key: 'telefonoReportante', width: 18 },
    { header: 'Conclusión',         key: 'conclusion',         width: 50 },
  ...
  titulo.value = `REPORTES SIN NOVEDAD${desde ? ` — ${desde} AL ${hasta}` : ''}`
  ...
  data.forEach((r, i) => {
    ...
    const vals = [r.reporte, r.nombreReportante, r.telefonoReportante, r.conclusion, r.fecha]
    vals.forEach((val, ci) => {
  ...
  return new NextResponse(new Uint8Array(buffer), {
    headers: {
      ...
      'Content-Disposition': `attachment; filename="reportes_sin_novedad_${fecha}.xlsx"`,
    }
  })
}