...
export async function GET(req: NextRequest) {
  ...
  const chequeo = await verificarAccesoAuxiliarApi(
    session.user.id,
    "auxiliar_cuestionario_robo",
    "ver",
  );
  ...
  if (formato === "pdf") {
    ...
    doc.setFontSize(13);
    doc.text("SECRETARÍA DE SEGURIDAD PÚBLICA MUNICIPAL", W / 2, 20, {
      align: "center",
    });
    ...
    doc.setFontSize(10);
    doc.text(
      "CUESTIONARIO ÚNICO DE ROBO — ROL DE AUXILIAR DE NOVEDADES",
      W / 2,
      66,
      { align: "center" },
    );
    ...
    doc.text(`Total de registros: ${datos.length}`, W - M, 88, {
      align: "right",
    });
    ...
    datos.forEach((r, idx) => {
      ...
      valores.forEach((val, i) => {
        ...
        doc.text(String(val), colX[i] + 3, y + 10, {
          maxWidth: cols[i].w * tableW - 6,
        });
      });
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(8);
    doc.text(`TOTAL DE REGISTROS: ${datos.length}`, W - M, y + 11, {
      align: "right",
    });
  ...
  inst.value =
    "SECRETARÍA DE SEGURIDAD PÚBLICA MUNICIPAL · SAN JUAN DEL RÍO, QRO.";
  inst.font = {
    name: "Arial",
    bold: true,
    size: 11,
