...
import { getMedidasFiltradas } from '@/lib/prevencion/repository'
import { createMedidaApi } from '@/lib/prevencion/actions'
...
export async function GET(request: NextRequest) {
  ...
  const rows = await getMedidasFiltradas(autoridad, status)
...
export async function POST(request: NextRequest) {
  ...
  const created = await createMedidaApi(body)
