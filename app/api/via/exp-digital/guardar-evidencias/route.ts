...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(
      `[VIA][EXP-DIGITAL][EVIDENCIAS] Iniciando subida de ${evidencias.length} evidencia(s). host=${host} sistema=${sistema}`,
    );
    ...
    for (const [index, evidencia] of evidencias.entries()) {
      ...
      console.log(
        `[VIA][EXP-DIGITAL][EVIDENCIAS] Subiendo evidencia ${index + 1}/${evidencias.length}: ${archivoRenombrado.name}`,
      );
      ...
      console.log(
        `[VIA][EXP-DIGITAL][EVIDENCIAS] Evidencia ${index + 1} subida. Ruta: ${responseJson.data.ruta_relativa}`,
      );
    }
