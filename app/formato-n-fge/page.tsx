...
export default async function FormatoNFgePage({
  searchParams,
}: {
  ...
  const registros = await listarFge(periodoValido)
  const user = session.user as { name: string; apellido?: string; email: string }

  return (
    <div style={{ ...pageWrap, display: 'flex', flexDirection: 'column' }}>
      <style>{fontsImport}</style>
      <DashboardHeader user={user} roleLabel="Eventos FGE" backHref="/envio-de-formatos" backLabel="Envío de Formatos" />
      ...
      <main className="pad-pagina" style={{ width: '100%', flex: 1, display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 16, alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            <Link href="/formato-n-fge" style={filtroBtn(!periodoFilter)}>TODOS</Link>
            <Link href="/formato-n-fge?periodo=diario" style={filtroBtn(periodoFilter === 'diario')}>DIARIO</Link>
            <Link href="/formato-n-fge?periodo=semanal" style={filtroBtn(periodoFilter === 'semanal')}>SEMANAL</Link>
            <Link href="/formato-n-fge?periodo=mensual" style={filtroBtn(periodoFilter === 'mensual')}>MENSUAL</Link>
          </div>
        </div>
