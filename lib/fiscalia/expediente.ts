const EXP_FISCALIA_HOST = process.env.EXPEDIENTE_HOST ?? 'https://sanjuandelrio.sytes.net:3044'
const EXP_FISCALIA_CODIGO = process.env.EXPEDIENTE_CODIGO_INVITACION ?? 'INV-2026-001'
...
export async function subirArchivoFiscalia(
  ...
  if (!response.ok) {
    ...
    console.error('[EXPEDIENTE DIGITAL] Error subiendo documento fiscalia:', error)
