...
import {
  ...
  obtenerTokenGuest,
} from "@/lib/shared/infracciones";
import type {
  UserInfo,
  LiberacionesResponse,
...
export async function capturarInfractorAction(body: {
  ...
  try {
    ...
    const {
      ...
      apellido_paterno_infractor: apellidoP,
      apellido_materno_infractor: apellidoM,
    ...
    const updateResult = await query<{
      ...
      `UPDATE via.v2_infracciones
       SET nombre_infractor = COALESCE($2, nombre_infractor),
           ...
           es_titular = $6,
           nombre_titular_liberacion = CASE WHEN $6 = true THEN COALESCE($2, nombre_titular_liberacion) ELSE COALESCE($2, 'NO_DATA') END,
           appaterno_titular_liberacion = CASE WHEN $6 = true THEN COALESCE($3, appaterno_titular_liberacion) ELSE COALESCE($3, 'NO_DATA') END,
           apmaterno_titular_liberacion = CASE WHEN $6 = true THEN COALESCE($4, apmaterno_titular_liberacion) ELSE COALESCE($4, 'NO_DATA') END,
           correo_titular_liberacion = CASE WHEN $6 = true THEN COALESCE($5, correo_titular_liberacion) ELSE COALESCE($5, 'NO_DATA') END,
           estatus = 'REGISTRADA',
    ...
    const msg =
      err instanceof Error ? err.message : "Error al capturar infractor";
    console.error("[capturarInfractorAction]", msg);
    return { success: false, error: msg };
  }
}

export async function obtenerDocumentosLiberacion(
  infraccionId: string,
): Promise<{
...
export async function revisarDocumentoAction(body: {
  ...
  try {
    ...
    if (accion !== "ACEPTADO" && accion !== "RECHAZADO")
      return { error: "accion debe ser ACEPTADO o RECHAZADO" };
...
export async function finalizarRevisionAction(infraccionId: string): Promise<{
  ...
  try {
    ...
    if (solicitudRes.rows.length === 0)
      return { error: "No se encontró solicitud de liberación" };
    ...
    if (docsRes.rows.length === 0)
      return { error: "No hay documentos asociados a la solicitud" };

    const tienePendientes = docsRes.rows.some(
      (d: Record<string, unknown>) =>
        !d.estatus_revision || d.estatus_revision === "PENDIENTE",
    );
    if (tienePendientes)
      return {
    ...
    if (nuevoEstatus === "PENDIENTE_PAGO") {
      ...
      if (infraRes.rows.length > 0) {
        ...
        nombre_usuario = (row.nombre_titular_liberacion ||
        ...
        apellidos_usuario =
          [
            ...
            .join(" ")
            .trim() || "SIN APELLIDO";
        correo_infractor = (row.correo_titular_liberacion ||
...
export async function generarOrdenPagoAction(payload: {
  ...
  try {
    ...
    if (!responseSA7.ok) {
      const rawResponse = await responseSA7.text();
      ...
      const message = String(
        sa7Response?.message ?? "Error al generar la orden de pago",
      );
      console.error(
        "[generarOrdenPagoAction] SA7 respondió con error:",
        responseSA7.status,
        rawResponse,
      );
    ...
    if (!orden_pago_id) {
      console.error(
        "[generarOrdenPagoAction] SA7 no devolvió orden_pago_id en headers",
      );
      return {
        ok: false,
        message: "El servicio de pagos no devolvió una orden de pago válida",
      };
    }
